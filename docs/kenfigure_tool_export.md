...
At the completion of the beta period, the app will be published as a free and publicly available
Canvas app.
...
4. Click **Install**. You will see the following screen. At this point, the app in installed,
...
1. Using the **Configuration Migration** tool (Settings > Configuration migration), export your entire organization or a subset.
   ...
   can make the export file size very large if you have many templates.
...
6. Unzip the file into a temporary directory or directly into your git repo (ideally after having created a branch).
   ...
   - In addition to the Kenfigure format files, you will also see a file called `schema_lint_errors.log`.
     ...
     Schema Lint generally implements the style guidance from the [Schema Design Style Guide](./schema_design_style_guide.md).

## Data privacy
The app needs no special permissions other than being a member of an Organization.
It does not access any data or configuration on the tenant. The only data the app
